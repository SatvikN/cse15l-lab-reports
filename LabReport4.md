...
* After moving any modifications to the staging area, I used the `git commit` command to record the changes within the repository. Since there is only 1 `git` subcommand that begins with _co_ I was able to finish the command by pressing `<tab>`.
